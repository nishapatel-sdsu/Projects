...
void  ExpressTable :: validateLiteral(string exp)
    ...
    if(!error){
        ...
        bool found =  literallist.retrieve(exp, searchliteral);//check to see if the literal is already present in the list
        if(!found){
            literallist.insert(litobj);
            }
        else {
            }
        }
...
void ExpressTable ::  evaluateNumber(string exp){
    bool error = false;
    string  symbol = "";
    int value = 0;
...
void ExpressTable ::  evaluateSymbol(string exp){
    bool error = false;
    string  symbol = "";
    int value = 0;
    int relocatableFlag = 0;
...
void ExpressTable ::  evaluateNumberOrSymbol(string orgexp,string exp,int indirect,int immediate, int indexed){
    bool error = false;
    string  symbol = "";
    int value = 0;
    int relocatableFlag = 0;
...
void ExpressTable :: evaluateImmediateExp(string exp){
    string  symbol = "";
    int indirectFlag = 0;
    int immediateFlag = 1;
    int indexFlag = 0;
    ...
    if(symbol.find("+") != string::npos){    //check whether it contains two operans with a + sign
        evaluateSimpleSumExp(exp,symbol,indirectFlag,immediateFlag,indexFlag);
        }
    else if(symbol.find("-") != string::npos){   //check whether it contains two operans with a - sign
        evaluateSimpleDiffExp(exp,symbol,indirectFlag,immediateFlag,indexFlag);
        }
    else {                                       //its a simple number or symbol
        evaluateNumberOrSymbol(exp,symbol,indirectFlag,immediateFlag,indexFlag); 
        }
    }
...
void ExpressTable :: evaluateIndirectExp(string exp) {
    string  symbol = "";
    int value = 0;
    int relocatableFlag = 0;
    int indirectFlag = 1;
    ...
    if(symbol.find("+") != string::npos){     //check whether it contains two operans with a + sign
        ...
        evaluateNumberOrSymbol(exp,symbol,indirectFlag,immediateFlag,indexFlag); //its a simple number or symbol
        }
    }
...
void ExpressTable :: evaluateIndexExp(string exp) {
    string  symbol = "";
    int relocatableFlag = 0;
    int indirectFlag = 0;
    int immediateFlag = 0;
...
void ExpressTable :: evaluateSimpleSumExp(string orgexp,string exp,int indirect,int immediate, int indexed) {
    bool error = false;
...
void ExpressTable :: evaluateSimpleDiffExp(string orgexp,string exp,int indirect,int immediate, int indexed) {
    bool error = false;
    string  symbol = "";
    char firstChar = exp[0];
    int value = 0;
...
void ExpressTable ::validateExp(string exp)
    {
    char firstChar = exp[0];
